...
class MusicOrganizer:
        ...
        for idx, folder in enumerate(folders, 1):
            try:
                ...
                if metadata.get('total_files', 0) == 0:
                    console.print("[yellow]No music files found in this folder, skipping...[/yellow]")
                    stats['skipped'] += 1
                    continue
    ...
    def _get_llm_proposal(self, metadata: Dict, user_feedback: Optional[str] = None) -> Dict:
...
Provide ONLY the JSON response."""
    ...
    def _fallback_proposal(self, metadata: Dict) -> Dict:
        ...
        return {
            'artist': analysis.get('common_artist', metadata.get('folder_name', 'Unknown Artist')),
            'album': analysis.get('common_album', metadata.get('folder_name', 'Unknown Album')),
            'year': analysis.get('common_year', '2023'),
            'release_type': 'Compilation' if analysis.get('likely_compilation') else 'Album',
            'confidence': 'low',
    ...
    def _organize_folder(self, source_folder: Path, proposal: Dict):
        ...
        console.print(f"[green]✓ Organized {copied} files to: {album_dir.relative_to(self.target_dir)}[/green]")
