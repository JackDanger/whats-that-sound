...
class TestMusicOrganizer:
    ...
    def test_init_success(self, mock_metadata_class, mock_ui_class, mock_llama_class, tmp_path):
    ...
    def test_build_prompt(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
        ...
        with patch('src.organizer.Llama', return_value=mock_llm):
            with patch('src.organizer.InteractiveUI', return_value=mock_ui):
                with patch('src.organizer.MetadataExtractor', return_value=mock_metadata_extractor):
                    organizer = MusicOrganizer(tmp_path / "model.gguf", tmp_path, tmp_path)
                    
                    metadata = mock_metadata_extractor.extract_folder_metadata.return_value
    ...
    def test_build_prompt_with_feedback(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
        ...
        with patch('src.organizer.Llama', return_value=mock_llm):
            with patch('src.organizer.InteractiveUI', return_value=mock_ui):
                with patch('src.organizer.MetadataExtractor', return_value=mock_metadata_extractor):
                    ...
                    metadata = mock_metadata_extractor.extract_folder_metadata.return_value
                    prompt = organizer._build_prompt(metadata, "This is actually a live album")
                    
                    assert 'User Feedback: This is actually a live album' in prompt
    
    def test_parse_llm_response_valid_json(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
    ...
    def test_parse_llm_response_invalid_json(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
        ...
        with patch('src.organizer.Llama', return_value=mock_llm):
            with patch('src.organizer.InteractiveUI', return_value=mock_ui):
                with patch('src.organizer.MetadataExtractor', return_value=mock_metadata_extractor):
                    ...
                    response = "I think the artist is The Beatles and the album is Abbey Road"
    ...
    def test_sanitize_filename(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
    ...
    @patch('src.organizer.MetadataExtractor')
    def test_organize_folder(self, mock_metadata_class, mock_copy, tmp_path, mock_llm, mock_ui):
        """Test organizing a folder."""
        # Set up MetadataExtractor mock with SUPPORTED_FORMATS
        mock_metadata_class.SUPPORTED_FORMATS = {'.mp3', '.flac', '.m4a', '.mp4', '.ogg', '.opus', '.wav'}
    ...
    def test_organize_no_folders(self, tmp_path, mock_llm, mock_ui, mock_metadata_extractor):
