...
class TestDirectoryAnalyzerIntegration:
    ...
    def test_analyze_real_multi_disc_structure(self, analyzer, tmp_path):
        ...
        disc1_info = next(sub for sub in analysis["subdirectories"] if sub["name"] == "Disc 1")
        disc2_info = next(sub for sub in analysis["subdirectories"] if sub["name"] == "Disc 2")
...
class TestStructureClassifierIntegration:
    ...
    def test_classify_artist_collection_with_realistic_response(self, classifier, mock_llm):
        ...
        mock_llm.return_value = {
            "choices": [{"text": "artist_collection"}]
        }
        ...
        structure = {
            ...
            "direct_music_files": 0,
            "subdirectories": [
                {"name": "Led Zeppelin I (1969)", "music_files": 9, "subdirectories": []},
                {"name": "Led Zeppelin II (1969)", "music_files": 9, "subdirectories": []},
                {"name": "Led Zeppelin III (1970)", "music_files": 10, "subdirectories": []},
                {"name": "Led Zeppelin IV (1971)", "music_files": 8, "subdirectories": []},
            ],
            "max_depth": 1,
...
class TestFileOrganizerIntegration:
    ...
    def test_organize_with_subdirectories(self, organizer, tmp_path):
        ...
        assert (target_album_dir / "Disc 1" / "track1.mp3").read_text() == "disc1 track1"
        assert (target_album_dir / "Disc 2" / "track1.mp3").read_text() == "disc2 track1"
    ...
    def test_sanitize_filename_functionality(self, organizer, tmp_path):
        ...
        target_dir = organizer.target_dir / "Test__Artist_Name" / "Test_Album_Name_ (2023)"
...
class TestStateManagerIntegration:
    ...
    def test_filter_unorganized_folders(self, state_manager, tmp_path):
        ...
        proposal = {"artist": "Test", "album": "Test", "year": "2023", "release_type": "Album"}
        state_manager.save_proposal_tracker(folder2, proposal)
        
        # Filter folders
        all_folders = [folder1, folder2, folder3]
        unorganized, organized_count = state_manager.filter_unorganized_folders(all_folders)
    ...
    def test_collection_tracker_operations(self, state_manager, tmp_path):
        ...
        albums = [
            {"artist": "Artist", "album": "Album 1", "year": "2023", "release_type": "Album"},
            {"artist": "Artist", "album": "Album 2", "year": "2024", "release_type": "Album"},
        ]
...
class TestProgressTrackerIntegration:
    ...
    def test_real_progress_tracking(self, tracker):
        ...
        tracker.increment_successful({"artist": "Artist1", "album": "Album1", "year": "2023"})
...
class TestEndToEndIntegration:
    ...
    def test_complete_single_album_workflow(self, tmp_path):
        ...
        with patch("src.organizer.Llama") as mock_llm_class:
            ...
            organizer.proposal_generator.llm.return_value = {
                "choices": [{"text": '{"artist": "Test Artist", "album": "Test Album", "year": "2023", "release_type": "Album", "confidence": "high"}'}]
            }
            
            # Mock UI to accept the proposal
            organizer.ui.get_user_feedback = Mock(return_value={
                "action": "accept",
                "proposal": {
                    "artist": "Test Artist",
                    "album": "Test Album", 
    ...
    def test_complete_multi_disc_workflow(self, tmp_path):
        ...
        with patch("src.organizer.Llama") as mock_llm_class:
            ...
            organizer.proposal_generator.llm.return_value = {
                "choices": [{"text": '{"artist": "Multi Artist", "album": "Multi Album", "year": "2024", "release_type": "Album", "confidence": "high"}'}]
            }
