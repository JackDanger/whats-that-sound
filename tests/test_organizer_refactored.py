...
class TestMusicOrganizerRefactored:
    ...
    @patch("src.organizer.Llama")
    def test_organize_single_album_success(self, mock_llama_class, tmp_path, complete_structure_analysis):
        ...
        # Mock components
        organizer.state_manager.filter_unorganized_folders = Mock(return_value=([test_folder], 0))
        organizer.directory_analyzer.analyze_directory_structure = Mock(return_value=complete_structure_analysis)
        organizer.ui.display_structure_analysis = Mock()  # Mock UI display
        organizer.ui.display_completion_summary = Mock()  # Mock UI display
        organizer.structure_classifier.classify_directory_structure = Mock(return_value="single_album")
        organizer.album_processor.process_single_album = Mock(return_value=True)
        organizer.state_manager.load_tracker_data = Mock(return_value={
            "proposal": {"artist": "Test Artist", "album": "Test Album", "year": "2023"}
        })
        
        organizer.organize()
        
        # Verify the workflow
        organizer.directory_analyzer.analyze_directory_structure.assert_called_once_with(test_folder)
    ...
    @patch("src.organizer.Llama")
    def test_organize_multi_disc_album_success(self, mock_llama_class, tmp_path, complete_structure_analysis):
        ...
        organizer.ui.display_structure_analysis = Mock()  # Mock UI display
        organizer.ui.display_completion_summary = Mock()  # Mock UI display
        organizer.structure_classifier.classify_directory_structure = Mock(return_value="multi_disc_album")
    ...
    @patch("src.organizer.Llama")
    def test_organize_artist_collection_success(self, mock_llama_class, tmp_path, complete_structure_analysis):
        ...
        organizer.ui.display_structure_analysis = Mock()  # Mock UI display
        organizer.ui.display_completion_summary = Mock()  # Mock UI display
        organizer.structure_classifier.classify_directory_structure = Mock(return_value="artist_collection")
        organizer.collection_processor.process_artist_collection = Mock(return_value=True)
        organizer.state_manager.load_tracker_data = Mock(return_value={
            "albums": [{"artist": "Test Artist", "album": "Album 1", "year": "2023"}]
        })
    ...
    def test_organize_no_music_files(self, mock_llama_class, tmp_path):
        ...
        # Mock components
        organizer.state_manager.filter_unorganized_folders = Mock(return_value=([test_folder], 0))
        organizer.directory_analyzer.analyze_directory_structure = Mock(return_value={
            "total_music_files": 0,
    ...
    @patch("src.organizer.Llama")
    def test_organize_unknown_structure_type(self, mock_llama_class, tmp_path, complete_structure_analysis):
        ...
        organizer.ui.display_structure_analysis = Mock()  # Mock UI display
        organizer.ui.display_completion_summary = Mock()  # Mock UI display
        organizer.structure_classifier.classify_directory_structure = Mock(return_value="unknown_type")
    ...
    def test_organize_processing_error(self, mock_llama_class, tmp_path):
        ...
        organizer.state_manager.filter_unorganized_folders = Mock(return_value=([test_folder], 0))
        organizer.directory_analyzer.analyze_directory_structure = Mock(side_effect=Exception("Processing error"))
